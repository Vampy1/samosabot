...
client.login(email="pobzplq029@dhamsi.com",password="cherry1")
...
adm=[]
##admx=[""http://aminoapps.com/p/q9q5zug]
...
def on_group_member_join(data):
	if data.comId==cidy:
		try:
			...
			subclient.send_message(chatId=data.message.chatId,message=f"""❣️ 𝐖𝐞𝐥𝐜𝐨𝐦𝐞 𝐭𝐨 𝐨𝐮𝐫 𝐜𝐡𝐚𝐭 𝐚𝐧𝐝 𝐚𝐥𝐬𝐨 𝐭𝐡𝐚𝐧𝐤𝐬 𝐟𝐨𝐫 𝐣𝐨𝐢𝐧 𝐨𝐮𝐫 𝐜𝐡𝐚𝐭𝐫𝐨𝐨𝐦 ❤️ 𝐈 𝐡𝐨𝐩𝐞 𝐮 𝐞𝐧𝐣𝐨𝐲 𝐨𝐮𝐫 𝐜𝐡𝐚𝐭 𝐚𝐧𝐝 𝐩𝐥𝐞𝐚𝐬𝐞 𝐟𝐨𝐥𝐥𝐨𝐰 𝐨𝐮𝐫 𝐚𝐥𝐥 𝐫𝐮𝐥𝐞𝐬 ❣️.<${data.message.author.nickname}$>!\n""",embedId=data.message.author.userId,embedTitle=data.message.author.nickname,embedLink=f"ndc://x{cid}/user-profile/{data.message.author.userId}",embedImage=img,mentionUserIds=[data.message.author.userId])
...
def on_group_member_leave(data):
	if data.comId==cidy:
		try:
			subclient.send_message(chatId=data.message.chatId,message="""[BC]━━━━┅━━━┅━━━━
[C]「 OH NOO !! 」
...
ndc://g/user-profile/{d}""")
				...
				if x.lower() not in l:
					try:
						...
						if d<=1000:
							...
							subclient.send_message(chatId=data.message.chatId,message="[ci]Can't clear more than 1000")
					except Exception as e:
						print(e)
				else:
			...
			if x.lower()=="-end" and c==[]:
				try:
					...
					subclient.send_message(chatId=data.message.chatId,message=f"end command will not work, pls contact Vampy for further assistance")
					print(f"VC started")
				except Exception as e:
					print(e)
...
-info, -members, -global {userlink}, -pm, -cid,  -lock,-insta
...
-poke, -holdmyhand, -punchme, -cheerme, -hugme, -loveyou, -hibot, -wotd

[BC]Admin Only:- 

-join {chatlink}, -start, -vc, -inviteall, -clear, -alock, -rlock""")
					print(f"Info requested by {data.message.author.nickname}")
				except Exception as e:
					print(e)
			if x.lower()=="-client" and c==[]:
				try:
					subclient.send_message(chatId=data.message.chatId,message="""[cb]Gabbi didnt add that command yet, please PM/DM him""")
					print(f"Info requested by {data.message.author.nickname}")
				except Exception as e:
					print(e)
...
[i]I m here for you...""")
			...
			if x.lower()=="-insta" and c==[]:
				try:
					subclient.send_message(chatId=data.message.chatId,message="""https://www.instagram.com/vampy915?r=nametag""")
					print(f"Info requested by {data.message.author.nickname}")
				except Exception as e:
					print(e)
